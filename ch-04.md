...
```
这两个版本中的任何一个，都表现出了比之前声明式代码更为突出的命令式的风格。读者在这两段代码中对*如何做*的关注度并不会不亚于*是什么*。(TODO: 此段存疑)
...
```
Note：不要忘记了，这里的`extractName(..)`还没有提取任何东西。我们只是对`prop(..)`使用了部分应用，来创建了一个函数。这个函数等待着我们传入任意的对象，然后它将会提取出此对象的`"name"`属性的值。我们当然也可以用`curry(prop)("name")`来完成同样的事情。
...
```
这个代码片段就不那么冗长了，但我认为它比之前代码片段的可读性差些，因为每个操作都是他自己的变量。无论是哪种方式，组合都帮助我们实现了无值风格的代码。
