...
声明式的代码，就像前面代码片段中的解构赋值，它只关注一段代码的结果应该是什么。命令式的代码，就像刚才显示的手动分配形参变量的做法，则更注重于它是如何获得结果的。如果你以后再来阅读这段代码的时候，你必须在头脑中模拟执行一遍这段代码，来了解结果到底是什么样子的。从这些代码当然也能看出结果，但它终归不是那么清晰明了。
...
这个版本毫无疑问要更加啰嗦一点，但是我想要稍微为它说几句好话。很显然的，它的流控制的逻辑要更加简单明了。因为在每个给`retValue`赋值的分支之中，实际上这个变量是被保护着的，因为每个条件分支都会事先检查这个变量是否已经被赋值。（译注：只是声明但还未赋值的变量，它的值将会是`undefined`）

比起在函数中使用`return`来提前返回值，我们使用正常的流控制（`if`逻辑）来决定`retValue`的赋值运算。然后在最后我们会直接返回这个变量`return retValue`。

我并不是说你应该无条件的总是使用一个`return`，或者不要提前返回值，只是我想你应该对于流控制语句要更加小心，